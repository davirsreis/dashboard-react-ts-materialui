...
function LineChart({ data }) {
  ...
  Object.keys(data).forEach(ano => {
    Object.keys(data[ano]).forEach(mes => {
      const valorTotal = data[ano][mes].total;
  ...
  const options = {
    ...
    onClick: async (event) => {
      if (event.chart.tooltip.title.length[0] != '') {
        ...
        window.location.href = `http://localhost:3000/faturamento/mes?mes_selecionado=${mesSelecionado}&ano_selecionado=${anoSelecionado}`;
      }
    }
